(function ($) {
	...
	$('.contact-form').on('submit', function() {
		...
		if (validateEmail(contactForm.find('.contact-email').val()) && contactForm.find('.contact-email').val().length !== 0 && contactForm.find('.contact-name').val().length !== 0 && contactForm.find('.contact-message').val().length !== 0) {
			...
			$.ajax({
				type: "POST",
				url : "https://www.enformed.io/6r2zynq6",
				data: {
				...
				success: function() {
					contactForm.find('.contact-loading').fadeOut();
					contactForm.find('.contact-success').find('.message').html('Success! Thanks for contacting! Will be in touch as soon as possible');
	...
	function initialize_map() {
		if ($('.map').length) {
			var myLatLng = new google.maps.LatLng(12.9166, 77.6101);
			var mapOptions = {
				zoom: 15,
				center: myLatLng,
				scrollwheel: true,
